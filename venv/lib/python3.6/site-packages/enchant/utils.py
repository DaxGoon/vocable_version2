...
def raw_unicode(raw):
    ...
    This function takes a string containing unicode escape characters, and
    returns the corresponding unicode string. Useful for writing unicode string
    literals in your python source while being upwards- compatible with Python
    3. For example, instead of doing this:
...
def raw_bytes(raw):
    ...
    This is analogous to raw_unicode, but processes byte escape characters to
...
class EnchantStr(str):
    ...
    string/unicode objects and the underlying enchant library, which expects all
    strings to be UTF-8 character arrays. It is a subclass of the default string
    class 'str' - on Python 2.x that makes it an ascii string, on Python 3.x it
    is a unicode object.

    Initialise it with a string or unicode object, and use the encode()
    method to obtain an object suitable for passing to the underlying C library.
    When strings are read back into python, use decode(s) to translate them back
    into the appropriate python-level string type.

    This allows us to following the common Python 2.x idiom of returning
    unicode when unicode is passed in, and byte strings otherwise. It also lets
    the interface be upwards-compatible with Python 3, in which string objects
...
def printf(values,sep=" ",end="\n",file=None):
    ...
    This function is a simple Python2/Python3 compatibility wrapper for
    printing to stdout.
...
def trim_suggestions(word,suggs,maxlen,calcdist=None):
    ...
    If the list of suggested words is too long, you can use this function to
    trim it down to a maximum length. It tries to keep the "best" suggestions
    based on similarity to the original word.

    If the optional "calcdist" argument is provided, it must be a callable
    taking two words and returning the distance between them. It will be used to
    determine which words to retain in the list. The default is a simple
...
def get_default_language(default=None):
    ...
    This function uses the 'locale' module to try to determine the user's
    preferred language. The return value is as follows:

        * if a locale is available for the LC_MESSAGES category, that language
          is used
    ...
    Note that determining the user's language is in general only possible if
    they have set the necessary environment variables on their system.
...
def get_resource_filename(resname):
    ...
    but tries to avoid loading pkg_resources unless we're actually in an egg.
...
def win32_data_files():
    ...
    This function returns a list of the supporting data files available to
    the running version of PyEnchant. This is in the format expected by the
    data_files argument of the distutils setup function. It's very useful, for
    example, for including the data files in an executable produced by py2exe.
