...
class Trie:
    ...
    "Fuzzy matching" can be done by allowing a certain number of missteps when
    traversing the Trie.
    """
    ...
    def search(self,word,nerrs=0):
        ...
        This method searches the trie for the given <word>, making precisely
        <nerrs> errors. It returns a list of words found.
...
class PyPWL:
    """Pure-python implementation of Personal Word List dictionary. This class
    emulates the PWL objects provided by PyEnchant, but implemented purely in
    python.
    """
    
    def __init__(self,pwl=None):
        """PyPWL constructor. This method takes as its only argument the name of
        a file containing the personal word list, one word per line. Entries
        will be read from this file, and new entries will be written to it
        automatically.

        If <pwl> is not specified or None, the list is maintained in memory
        only.
    ...
    def add(self,word):
        """Add a word to the user's personal dictionary. For a PWL, this means
