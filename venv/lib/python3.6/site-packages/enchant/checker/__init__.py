...
class SpellChecker:
    ...
    This class is designed to implement a spell-checking loop over a block of
    text, correcting/ignoring/replacing words as required. This loop is
    implemented using an iterator paradigm so it can be embedded inside other
    loops of control.

    The SpellChecker object is stateful, and the appropriate methods must be
    called to alter its state and affect the progress of the spell checking
    session. At any point during the checking session, the attribute 'word' will
    hold the current erroneously spelled word under consideration. The action to
    take on this word is determined by calling methods such as 'replace',
    'replace_always' and 'ignore_always'. Once this is done, calling 'next'
    advances to the next misspelled word.

    As a quick (and rather silly) example, the following code replaces each
    misspelled word with the string "SPAM":
    ...
    unicode) character elements. This allows the in-place modification of the
    string as it is checked, and is the closest thing Python has to a mutable
    string. The text can be set as any of a normal string, unicode string,
    character array or unicode character array. The 'get_text' method will
    return the modified array object if an array is used, or a new string object
    if a string it used.

    Words input to the SpellChecker may be either plain strings or unicode
    objects. They will be converted to the same type as the text being checked,
    using python's default encoding/decoding settings.

    If using an array of characters with this object and the array is
    modified outside of the spellchecking loop, use the 'set_offset' method to
    reposition the internal loop pointer to make sure it doesn't skip any words.
    """
    ...
    def __init__(self,lang=None,text=None,tokenize=None,chunkers=None,filters=None):
        ...
        SpellChecker objects can be created in two ways, depending on the
        nature of the first argument. If it is a string, it specifies a language
        tag from which a dictionary is created. Otherwise, it must be an enchant
        ...
        If <tokenize> is not given and the first argument is a Dict, its
        'tag' attribute must be a language tag so that a tokenization function
        can be created automatically. If this attribute is missing the user's
    ...
    def set_text(self,text):
        ...
        This method must be called, or the 'text' argument supplied to the
        constructor, before calling the 'next()' method.
    ...
    def wants_unicode(self):
        ...
        This method will return True if the checker wants unicode strings as
        input, False if it wants normal strings. It's important to provide the
        correct type of string to the checker.
        """
    ...
    def coerce_string(self,text,enc=None):
        ...
        This method can be used to automatically ensure that strings are of
        the correct type required by this checker - either unicode or standard.
        If there is a mismatch, conversion is done using python's default
        encoding unless another encoding is specified.
    ...
    def next(self):
        ...
        This method is designed to support the iterator protocol. Each time
        it is called, it will advance the 'word' attribute to the next spelling
        error in the text. When no more errors are found, it will raise
        StopIteration.

        The method will always return self, so that it can be used sensibly
        in common idioms such as:
    ...
    def replace_always(self,word,repl=None):
        ...
        If a single argument is given, this is used to replace the current
        erroneous word. If two arguments are given, that combination is added to
    ...
    def set_offset(self,off,whence=0):
        ...
        For more details on the purpose of the tokenization offset, see the
        documentation of the 'enchant.tokenize' module. The optional argument
        whence indicates the method by which to change the offset:
    ...
    def leading_context(self,chars):
        ...
        This method returns up to <chars> characters of leading context - the
        text that occurs in the string immediately before the current erroneous
        word.
    ...
    def trailing_context(self,chars):
        ...
        This method returns up to <chars> characters of trailing context -
        the text that occurs in the string immediately after the current
        erroneous word.
