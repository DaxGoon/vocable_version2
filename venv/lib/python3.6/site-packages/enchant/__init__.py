...
class _EnchantObject(object):
    ...
    This class implements some general functionality for interfacing with the
    '_enchant' C-library in a consistent way. All public objects from the
    'enchant' module are subclasses of this class.

    All enchant objects have an attribute '_this' which contains the pointer
    to the underlying C-library object. The method '_check_this' can be called
    to ensure that this point is not None, raising an exception if it is.
    """
    ...
    def _raise_error(self,default="Unspecified Error",eclass=Error):
         ...
         This method causes an Error to be raised. Subclasses should override
         it to retrieve an error indication from the underlying API if possible.
         If such a message cannot be retrieved, the argument value <default> is
         used. The class of the exception can be specified using the argument
         <eclass>
...
class Broker(_EnchantObject):
    ...
    objects directly. The 'enchant' module provides a default broker object so
    that 'Dict' objects can be created directly.
    ...
    def __init__(self):
        ...
        This method is the constructor for the 'Broker' object. No arguments
        are required.
        """
    ...
    def _free(self):
        ...
        associated with a Broker object. It is called automatically when the
        object is garbage collected. If called explicitly, the Broker and any
        associated Dict objects must no longer be used.
        """
    ...
    def request_dict(self,tag=None):
        ...
        This method constructs and returns a Dict object for the requested
        language. 'tag' should be a string of the appropriate form for
        specifying a language, such as "fr" (French) or "en_AU" (Australian
        English). The existence of a specific language can be tested using the
        'dict_exists' method.

        If <tag> is not given or is None, an attempt is made to determine the
        current language in use. If this cannot be determined, Error is raised.
    ...
    def _request_dict_data(self,tag):
        ...
        This method call passes on the call to the C library, and does some
    ...
    def request_pwl_dict(self,pwl):
        ...
        This method behaves as 'request_dict' but rather than returning a
        dictionary for a specific language, it returns a dictionary referencing
        a personal word list. A personal word list is a file of custom
        dictionary entries, one word per line.
    ...
    def _free_dict(self,dict):
        ...
        This method frees system resources associated with a Dict object. It
        is equivalent to calling the object's 'free' method. Once this method
        has been called on a dictionary, it must not be used again.
    ...
    def dict_exists(self,tag):
        ...
        This method checks whether there is a dictionary available for the
        language specified by 'tag'. It returns True if a dictionary is
    ...
    def set_ordering(self,tag,ordering):
        ...
        and multiple languages. This method specifies which dictionaries the
        broker should prefer when dealing with a given language. 'tag' must be
        an appropriate language specification and 'ordering' is a string listing
        the dictionaries in order of preference. For example a valid ordering
        might be "aspell,myspell,ispell". The value of 'tag' can also be set to
        "*" to set a default ordering for all languages for which one has not
        been set explicitly.
    ...
    def describe(self):
        ...
        This method returns a list of descriptions of each of the dictionary
        providers available. Each entry in the list is a ProviderDesc object.
        """
        self._check_this()
    ...
    def __describe_callback(self,name,desc,file):
        ...
        'enchant_broker_describe'. It collects the given arguments in a tuple
        and appends them to the list '__describe_result'.
    ...
    def list_dicts(self):
        ...
        This method returns a list of dictionaries available to the broker.
        Each entry in the list is a two-tuple of the form:

            (tag,provider)

        where <tag> is the language lag for the dictionary and <provider> is
        a ProviderDesc object describing the provider through which that
    ...
    def __list_dicts_callback(self,tag,name,desc,file):
        ...
        This method is used as a callback into the _enchant function
        'enchant_broker_list_dicts'. It collects the given arguments into an
        appropriate tuple and appends them to '__list_dicts_result'.
    ...
    def list_languages(self):
        ...
        This function returns a list of language tags for which a dictionary
        is available.
        """
        langs = []
    ...
    def __describe_dict(self,dict_data):
        """Get the description tuple for a dict data object. <dict_data> must be
        a C-library pointer to an enchant dictionary. The return value is a
        tuple of the form:
    ...
    def set_param(self,name,value):
        ...
        Parameters are used to provide runtime information to individual
        provider backends. For example, the myspell provider will search any
        directories given in the "enchant.myspell.dictionary.path" parameter
...
class Dict(_EnchantObject):
    ...
    Dictionary objects are responsible for checking the spelling of words and
    suggesting possible corrections. Each dictionary is owned by a Broker
    object, but unless a new Broker has explicitly been created then this will
    be the 'enchant' module default Broker and is of little interest.
    ...
    Information about the dictionary is available using the following :ivar *
    tag: the language tag of the dictionary :ivar * provider: a ProviderDesc
    object for the dictionary provider
    """

    def __init__(self,tag=None,broker=None):
        """Dict object constructor.

        A dictionary belongs to a specific language, identified by the string
        <tag>. If the tag is not given or is None, an attempt to determine the
        language currently in use is made using the 'locale' module. If the
        current language cannot be determined, Error is raised.

        If <tag> is instead given the value of False, a 'dead' Dict object is
        created without any reference to a language. This is typically only
        useful within PyEnchant itself. Any other non-string value for <tag>
        raises Error.

        Each dictionary must also have an associated Broker object which
        obtains the dictionary information from the underlying system. This may
        be specified using <broker>. If not given, the default broker is used.
    ...
    def _switch_this(self,this,broker):
        ...
        pointer, it is very convenient to allow this to be switched at run-time.
        Pass a new dict data object into this method to affect the necessary
        changes. The creating Broker object (at the Python level) must also be
        provided.

        This should *never* *ever* be used by application code. It's a
        convenience for developers only, replacing the clunkier <data> parameter
        to __init__ from earlier versions.
    ...
    def _free(self):
        ...
        This method frees underlying system resources for a Dict object. Once
        it has been called, the Dict object must no longer be used. It is called
    ...
    def check(self,word):
        ...
        This method takes a word in the dictionary language and returns True
        if it is correctly spelled, and false otherwise.
    ...
    def suggest(self,word):
        ...
        This method tries to guess the correct spelling for a given word,
        returning the possibilities in a list.
    ...
    def store_replacement(self,mis,cor):
        ...
        miss-spelled word <mis> is in fact correctly spelled as <cor>. Such a
        suggestion will typically mean that <cor> appears early in the list of
        suggested spellings offered for later instances of <mis>.
    ...
    def __describe_callback(self,tag,name,desc,file):
        ...
        This method is used as a callback into the _enchant function
        'enchant_dict_describe'. It collects the given arguments in a tuple and
        stores them in the attribute '__describe_result'.
...
class DictWithPWL(Dict):
    ...
    NOTE:  As of version 1.4.0, enchant manages a per-user pwl and
        exclude list. This class is now only needed if you want to explicitly
        maintain a separate word list in addition to the default one.

    This class behaves as the standard Dict class, but also manages a
    personal word list stored in a separate file. The file must be specified at
    creation time by the 'pwl' argument to the constructor. Words added to the
    dictionary are automatically appended to the pwl file.
    ...
    If either 'pwl' or 'pel' are None, an in-memory word list is used. This
    will prevent calls to add() and remove() from affecting the user's default
    word lists.

    The Dict object managing the PWL is available as the 'pwl' attribute. The
    Dict object managing the PEL is available as the 'pel' attribute.

    To create a DictWithPWL from the user's default language, use None as the
    'tag' argument.
    """
    ...
    def __init__(self,tag,pwl=None,pel=None,broker=None):
        ...
        The argument 'pwl', if not None, names a file containing the personal
        word list. If this file does not exist, it is created with default
        permissions.

        The argument 'pel', if not None, names a file containing the personal
        exclude list. If this file does not exist, it is created with default
        permissions.
    ...
    def check(self,word):
        ...
        This method takes a word in the dictionary language and returns True
        if it is correctly spelled, and false otherwise. It checks both the
        dictionary and the personal word list.
