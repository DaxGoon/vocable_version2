...
class tokenize:
    ...
    Each tokenizer must be an iterator and provide the 'offset' attribute as
    described in the documentation for this module.

    While tokenizers are in fact classes, they should be treated like
    functions, and so are named using lower_case rather than the CamelCase more
    traditional of class names.
    """
...
class basic_tokenize(tokenize):
    ...
    This tokenizer does the most basic thing that could work - it splits text
    into words based on whitespace boundaries, and removes basic punctuation
    symbols from the start and end of each word.
    """
    _DOC_ERRORS = []
...
def wrap_tokenizer(tk1,tk2):
    ...
    This function takes two tokenizer functions 'tk1' and 'tk2', and returns
    a new tokenizer function that passes the output of tk1 through tk2 before
    yielding it to the calling code.
...
class Chunker(tokenize):
    ...
    A chunker is designed to chunk text into large blocks of tokens. It has
    the same interface as a tokenizer but is for a different purpose.
    """
    pass
...
class Filter(object):
    ...
    A filter is designed to wrap a tokenizer (or another filter) and do two
    things:
    ...
    method _skip(word) may be overridden to return True for words that should be
    skipped, and false otherwise. The method _split(word) may be overridden as
    tokenization function that will be applied to further tokenize any words
    that aren't skipped.
    """
    ...
    def _skip(self,word):
        ...
        If this method returns true, the given word will be skipped by the
        filter. This should be overridden in subclasses to produce the desired
        functionality. The default behaviour is not to skip any words.
    ...
    def _split(self,word):
        ...
        This method must be a tokenization function that will split the given
        word into sub-tokens according to the needs of the filter. The default
        behaviour is not to split any words.
    ...
    class _TokenFilter(object):
        ...
        This might seem convoluted, but we're trying to create something akin
        to a meta-class - when Filter(tknzr) is called it must return a
        *callable* that can then be applied to a particular string to perform
        the tokenization. Since we need to manage a lot of state during
        tokenization, returning a class is the best option.
        """
        _DOC_ERRORS = ["tknzr"]
...
class URLFilter(Filter):
    """Filter skipping over URLs. This filter skips any words matching the
...
class WikiWordFilter(Filter):
    """Filter skipping over WikiWords. This filter skips any words matching the
    following regular expression:
...
class EmailFilter(Filter):
    """Filter skipping over email addresses. This filter skips any words
...
class MentionFilter(Filter):
    """Filter skipping over @mention. This filter skips any words matching the
    following regular expression:
...
class HashtagFilter(Filter):
    """Filter skipping over #hashtag. This filter skips any words matching the
    following regular expression:
...
class HTMLChunker(Chunker):
    ...
    The operation of this chunker is very simple - anything between a "<" and
    a ">" will be ignored. Later versions may improve the algorithm slightly.
    """
